"""
Check that standalone Shell comments follow the period punctuation rule.

usage:
  python scripts/shell/check_comment_period.py [FILE ...]
"""
...
def _collect_standalone_comments(path: Path) -> list[tuple[int, int, str]]:
    ...
    for line_num, line in enumerate(lines, start=1):
        ...
        if stripped.startswith(_COMMENT_PREFIX) and not stripped.startswith(
            _SHEBANG_PREFIX
        ):
