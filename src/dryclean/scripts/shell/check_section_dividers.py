"""
Check that no Shell file contains section divider comments.

usage:
  python scripts/shell/check_section_dividers.py [FILE ...]
"""
...
def _analyze_file(path: Path) -> list[_FileResult]:
    ...
    return [
        ...
        if _DIVIDER_PATTERN.match(line) and not line.strip().startswith(_SHEBANG_PREFIX)
    ]
