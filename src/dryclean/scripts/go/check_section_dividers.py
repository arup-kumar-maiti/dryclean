"""
Check that no Go function contains section divider comments.

usage:
  python scripts/go/check_section_dividers.py [FILE ...]
"""
...
def _find_function_ranges(lines: list[str]) -> list[_FunctionRange]:
    ...
    for i, line in enumerate(lines):
        if _FUNC_PATTERN.match(line.strip()) and not in_function:
            function_start = i + 1
            in_function = True
            brace_depth = 0
