"""
Check that all Go source files use snake_case names.

usage:
  python scripts/go/check_snake_case_filename.py [FILE ...]
"""
...
_SNAKE_CASE_PATTERN = re.compile(r"^[a-z][a-z0-9_]*(_test)?\.go$")


def _is_snake_case(path: Path) -> bool:
    return bool(_SNAKE_CASE_PATTERN.match(path.name))
